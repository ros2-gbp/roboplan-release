...
Sampling-based planners search for a collision-free path between a start and goal configuration by randomly sampling the configuration space and building a tree (or trees) of valid motions.
They treat the collision checker as a black box, which makes them well suited to high-dimensional problems such as articulated arms, where explicitly representing the free space is intractable.
...
Contributions of other variants and planners are welcome.
...
This pulls the tree toward the goal while still sampling randomly often enough to escape local minima around obstacles.
...
RRT-Connect typically converges much faster than single-tree RRT.
Goal biasing is disabled in this mode, since the ``CONNECT`` step already pulls the trees toward one another, so the planner samples uniformly at random.
...
RRT* adds an *optimization* step so that, given enough samples, the path converges toward the shortest one.
Enable it with ``rrt_star``; combined with ``rrt_connect``, both trees are optimized.
...
``rewire_distance`` is the configuration-space radius of the neighborhood considered when choosing parents and rewiring, in the same units as ``max_connection_distance``.
It should generally be at least as large as ``max_connection_distance`` so that nodes a single connection step away are considered; larger values yield shorter paths at the cost of more collision checks per node.

Asymptotic optimality requires the planner to keep sampling past the first solution; see :ref:`fast_return <fast-return>` below.
...
RRT* only improves the tree as it samples, so pair ``rrt_star = true`` with ``fast_return = false`` for asymptotically optimal paths.
With plain RRT or RRT-Connect, which never rewire, disabling ``fast_return`` only keeps the cheapest of the independently discovered paths, which rarely justifies the extra time.

Even with ``fast_return = true``, RRT* rewires as it grows, so its first path is already noticeably shorter than plain RRT's.
...
Many manipulation tasks restrict *how* the robot moves, not just where it ends up, such as carrying a full cup upright or sliding a drawer along its rail.
These are end-effector pose constraints that must hold at every point of the path, which a goal cannot express.
...
Position and orientation share one constraint because they share one frame Jacobian, so the planner's hot path runs forward kinematics once.
...
This differs from the paper, which always solves against all six task-space coordinates,
...
It has to be read together with ``path_step_size``, because two projected configurations are
...
rather than stopping as soon as it dips inside, which leaves headroom for the drift.
The defaults for both are chosen to be consistent with each other.

Note that tightening a bound does not tighten the tolerance: it is an absolute slack,
so a very small bound with the default tolerance is loose in relative terms.
Scale the tolerance and ``path_step_size`` down together when tightening a bound.
...
- ``rrt_star`` **is worth enabling.** Walking in small hops leaves a zigzagging path.
  ...
  ``path_step_size`` apart, so most candidates are short enough to be accepted.
  Those short rewires collapse the zigzag, giving roughly a quarter shorter paths on the
  bundled example.
...
- **Constraints can disconnect the space.** Bounding rotational degrees of freedom on a
  ...
  since crossing would mean tipping the frame over. Two valid configurations may then be
  unreachable from one another, and no amount of planning time will bridge them.
...
Every RRT iteration queries the tree for the node closest to a new sample.
RoboPlan delegates these queries to `dynotree <https://github.com/quimortiz/dynotree>`_, a header-only k-d tree library vendored into ``roboplan_rrt/include/dynotree``.

**k-d trees.**
dynotree partitions the state space with a k-d tree, so nearest-neighbor queries run in roughly logarithmic rather than linear time in the number of nodes.
The tree splits along whichever dimension currently has the widest spread, and its branch-and-bound search uses each state space's ``distance_to_rectangle`` routine to prune subtrees that cannot contain a closer neighbor.

**State spaces.**
A naive planner treats every joint as a coordinate in :math:`\mathbb{R}^n` with a plain Euclidean norm, which is incorrect for joints that are not a Euclidean line.
...
- **SE(2)** — the planar group :math:`\mathbb{R}^2 \times SO(2)`, used for planar joints such as mobile bases: a 2D translation plus a wrapping heading angle.
- **SO(3)** — the rotation group for 3D orientations. RoboPlan does not use it yet; floating joints are unsupported.
...
Nearest-neighbor results therefore respect the true configuration-space metric: a sample near :math:`+\pi` is found to be close to a tree node near :math:`-\pi`.
...
It is typically post-processed with :doc:`path shortcutting <path_shortcutting>` to remove detours, and then timed into a trajectory with :doc:`trajectory_generation`.

Visualization and Debugging
~~~~~~~~~~~~~~~~~~~~~~~~~~~

Inspecting how a tree explored the space helps when a plan fails, takes too long, or detours.
After planning, ``rrt.getNodes()`` returns the start and goal trees' nodes.

The :func:`visualizeTree` helper (in ``roboplan.rrt``) renders these trees in a `Viser <https://viser.studio>`_ scene.
For each tree edge, it runs forward kinematics along the connecting motion and draws the resulting end-effector path as line segments.
The start and goal trees are drawn in different colors.
